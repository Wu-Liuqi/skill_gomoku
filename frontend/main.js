...
// Prime fallback audio on first user gesture so mobile browsers can play tones
const setupMobileAudioUnlock = () => {
  if (typeof window === 'undefined') {
    return;
  }

  if (!audioManager.isSupported() || audioManager.supportsSpeech || !audioManager.AudioContextClass) {
    return;
  }
  ...
  const unlock = () => {
    if (unlocked || !audioManager.isEnabled()) {
      return;
    }
    unlocked = true;
    ...
    try {
      ...
      const ctx = audioManager.ensureAudioContext();
      ...
      if (ctx) {
        ...
        try {
          ...
          console.warn('Unable to prime audio context buffer', err);
        }
      }
    } catch (err) {
      console.warn('Unable to unlock audio context', err);
    }
